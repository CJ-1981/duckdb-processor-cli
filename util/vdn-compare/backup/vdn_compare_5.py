...
def main():
    ...
    for fmt in req_formats:
        ...
        if not true_mismatches.empty:
            ...
            # Dynamic Columns for sample report
            report_cols = ['vin']
            for col in existing_targets:
                if col == 'VDN_LIST':
                    report_cols.append('vdn_match')
                else:
                    target_name = 'sw' if col == 'CONSUMER_SW_VERSION' else ('model' if col == 'MODEL' else col.lower())
                    report_cols.extend([f'source_{target_name}_display', f'target_{target_name}_display', f'{target_name}_match'])
            
            report_cols.append('Result')
            if compare_vdn:
                report_cols.extend(['Only in Target (missing in Source)', 'Only in Source (missing in Target)'])
                
            cols_to_use = [c for c in report_cols if c in df_s.columns]
