...
def main():
    ...
    for col in ['VIN', 'CONSUMER_SW_VERSION', 'VDN_LIST', 'MODEL']:
    ...
    for df in (df_source, df_target):
        ...
        if 'MODEL' in df.columns:
            # Treat V216 and EX30 as equal by normalizing them to a single brand name
            df['MODEL'] = df['MODEL'].replace('V216', 'EX30')
    ...
    joined AS (
        SELECT
            ...
            CASE 
                ...
                ELSE COALESCE(s.source_sw, 'NO DATA')
            END as source_sw_display,
            CASE 
                WHEN t.vin IS NULL THEN 'VIN not found in Target'
                ELSE COALESCE(t.target_sw, 'NO DATA')
            END as target_sw_display,
            CASE 
                WHEN s.vin IS NULL THEN 'N/A'
                ELSE COALESCE(s.source_model, 'NO DATA')
            END as source_model_display,
            CASE 
                WHEN t.vin IS NULL THEN 'N/A'
                ELSE COALESCE(t.target_model, 'NO DATA')
            END as target_model_display,
            CASE 
            ...
            CASE 
                WHEN s.vin IS NULL OR t.vin IS NULL THEN 'MISMATCH'
                WHEN s.source_model = t.target_model THEN 'MATCH' 
                ELSE 'MISMATCH' 
            END as model_match,
            CASE 
                WHEN s.vin IS NULL OR t.vin IS NULL THEN 'MISMATCH'
                ...
                ELSE 'MISMATCH' 
            END as vdn_match,
            s.source_vdns_json as s_json,
            t.target_vdns_json as t_json
        FROM source_data s
        FULL OUTER JOIN target_data t ON s.vin = t.vin
    )
    SELECT
        vin,
        source_model_display as source_model,
        target_model_display as target_model,
        source_sw_display as source_sw,
        target_sw_display as target_sw,
        model_match,
        sw_match,
        vdn_match,
        CASE WHEN sw_match = 'MISMATCH' OR vdn_match = 'MISMATCH' OR model_match = 'MISMATCH' THEN 'NOK' ELSE 'OK' END as Result,
        s_json as s_vdns_json,
        t_json as t_vdns_json
    FROM joined
    {sort_clause}
    """
    ...
    def compute_diff(row):
        ...
        s_vdns_str = row['s_vdns_json']
        t_vdns_str = row['t_vdns_json']
    ...
    final_output = result_df.drop(columns=['s_vdns_json', 't_vdns_json'])
    ...
    if is_md:
        summary_lines = [
            ...
            f"- **VINs with Mismatched Model**: {len(final_output[final_output['model_match'] == 'MISMATCH'])}",
    ...
    if not mismatched_vdns.empty:
        ...
        cols = ['vin', 'source_model', 'target_model', 'model_match', 'source_sw', 'target_sw', 'sw_match', 'Result', 'vdn_match', 'Only in Target (missing in Source)', 'Only in Source (missing in Target)']
